...
| Веб-морда | ✅ Статус (WS), QR, настройки, push-уведомления (ntfy), лог сообщений |
...
В веб-морде есть раздел **«Лог сообщений»** — все входящие и исходящие через коннектор,
с вкладками по мессенджерам и кнопкой обновления.

Для каждого сообщения видно:
- **↓** (зелёный) — входящее от клиента / **↑** (синий) — исходящее к клиенту
- Чип мессенджера: **WA** / **MAX** / **TG**
- Номер телефона клиента
- Текст сообщения
- Время

Лог пишется в SQLite (`/data/bridge.sqlite3`, таблица `messages`) и хранится бессрочно.
Входящие (клиент → Битрикс), ответы из Open Line и исходящие через CRM-кнопку — все три направления.
**Важно:** лог содержит только сообщения с момента старта сервиса — исторических данных нет.

---

## Диагностика

### MAX: сообщение клиента не дошло в Битрикс

Если MAX-адаптер перезапускался в момент когда клиент отвечал — сообщение теряется.
pymax WebClient не воспроизводит пропущенные offline-сообщения при реконнекте.
Настройте ntfy-уведомления чтобы получать алерт при падении адаптера и быстро реагировать.
...
обновляет название и иконку коннектора, не удаляет привязку к линии и не трогает историю.
