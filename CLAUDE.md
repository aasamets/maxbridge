...
**Верифицировано (2026-08-05):**
- Веб-морда: лог сообщений с iOS segmented control + карточные строки (направление, чип, phone, text, time) ✅
- Иконки коннекторов в Битрикс24: белый глиф на цветном фоне без префикса «MaxBridge» ✅
...
В веб-морде — секция «Лог сообщений» с iOS-style segmented control и карточными строками:
- Сегментный контрол: Все / WhatsApp / MAX / Telegram (белая активная вкладка + тень)
- Строка: значок направления ↓↑ (зелёный/синий), чип мессенджера (WA/MAX/TG), phone + text, время
- Кнопка обновления (↻) справа от вкладок
- **Нет истории при первом запуске** — логируются только сообщения с момента старта core
...
**Закрыто (2026-08-05):** Таблица `messages` в SQLite, логирование входящих + исходящих.
UI: iOS segmented control + карточные строки с цветными бейджами направления и чипами мессенджера.
