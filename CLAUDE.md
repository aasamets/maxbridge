...
## ✅ РАБОЧИЙ КОСТЯК (верифицировано 2026-07-28, полный двусторонний WA)
...
**Верифицировано вручную (2026-07-28):**
- Клиент → WA → Битрикс24 Открытая линия → менеджер ✅
- Менеджер → Битрикс24 → WA → клиент (ответ в линии) ✅
- Менеджер инициирует первым: карточка контакта → «Сообщение» → MaxBridge → WhatsApp → клиент ✅
...
~~Симптом: выпадает «MaxBridge» без указания мессенджера~~
...
Каналы разделяются по `CODE` (`maxbridge_wa`, `maxbridge_max`) автоматически.
Дополнительных изменений в `install_connector.py` не нужно.
