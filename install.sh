...
# ── Секция 5: Мониторинг (опционально) ───────────────────────────────────────
step "Мониторинг через ntfy (опционально)"
echo -e "  ${Y}ntfy — push-уведомления при падении/восстановлении адаптеров.${N}"
echo -e "  Формат URL: https://ntfy.sh/ваш-топик  (или свой сервер)"
echo -e "  Оставьте пустым чтобы пропустить."
echo
read_val "ntfy URL (Enter — пропустить)" ""
NTFY_URL="$_VAL"
[[ -n "$NTFY_URL" ]] && ok "ntfy: $NTFY_URL" || ok "Мониторинг пропущен"
...
# ── Мониторинг ────────────────────────────────────────────────
# push-уведомления при падении/восстановлении адаптеров
NTFY_URL=${NTFY_URL}
EOF
ok ".env записан"
